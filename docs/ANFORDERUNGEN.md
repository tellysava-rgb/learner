...
- **Wirkung:** In `learn.php` und `drill.php` erscheint nach jeder beantworteten Karte ein `alert alert-info`-Panel (einmalig, wie eine Flash-Message) mit dem Vorher/Nachher-Status der gerade beantworteten Karte:
  ...
  - Drill: bei besonderen Ereignissen (gemeistert, als zu schwer markiert, Vormerkung erreicht) eine Antwort-Zeile ("gewusst"/"musste nachdenken") plus eine hervorgehobene Zeile mit Fach-/Zähler-Änderung; sonst **beide** Session-Zähler gleichzeitig, je auf eigener Zeile und mit der Bedeutung direkt dahinter _(Suffix statt separater Antwort-Zeile seit v3.7.8)_ — "Mastery-Zähler X/`DRILL_MASTERY_THRESHOLD` - gewusst" (Folge richtiger Antworten, setzt bei falscher Antwort auf 0 zurück) und "Zu-schwer-Zähler X/`DRILL_TOO_HARD_LIMIT` - nicht gewusst" (Gesamtzahl falscher Antworten in dieser Session, wird durch richtige Antworten dazwischen NICHT zurückgesetzt) _(v3.2.42)_. Im Normalfall gibt es damit keine eigenständige "gewusst"/"musste nachdenken"-Zeile mehr — die Antwort ergibt sich aus den Zählerständen.
