...
- **Verfügbarkeits-Hinweis auf der Leitner-Setup-Seite** _(v3.3.18, Formulierung vereinfacht v3.3.19)_: Ohne diesen Hinweis wirkte eine kleiner als die eingestellte Kartenanzahl ausgefallene Session wie ein Fehler, obwohl sie durch das Tageslimit beabsichtigt gedrosselt war (z.B. 25 Karten eingestellt, Warteschlange mit 119 Karten aber 0 bereits fällig → nur 10 tatsächlich verfügbar). Unter dem Feld "Kartenanzahl" steht jetzt live: "Heute maximal N Karten verfügbar (es besteht ein Tageslimit für neue Karten). X sind heute fällig + bis zu Y aus der Warteschlange." Bei Checkbox-Mehrfachauswahl aktualisiert sich die Zahl automatisch beim An-/Abwählen (Summe über alle ausgewählten Listen, serverseitig pro Liste vorberechnet, keine Nachlade-Anfrage nötig). Steht die eingestellte Kartenanzahl über der tatsächlich verfügbaren Zahl, wird der Hinweis farblich hervorgehoben (`text-warning`). Rein informativ — die Mechanik (Tageslimit, Drosselung neuer Karten) selbst bleibt unverändert, siehe Abschnitt oben.
