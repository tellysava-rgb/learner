...
- **Wirkung:** In `learn.php` und `drill.php` erscheint nach jeder beantworteten Karte ein `alert alert-info`-Panel (einmalig, wie eine Flash-Message) mit dem Vorher/Nachher-Status der gerade beantworteten Karte:
  ...
  - Drill, feste zweite Zeile in jeder Variante _(v3.7.2, Format überarbeitet v3.7.5, Total-Definition korrigiert v3.7.6)_: **"Karten der Session: T total · D im Deck · R Reserve · G gemeistert · P pausiert [· N vorgemerkt]"** — macht Pool-Begrenzung und Reserve-Nachschub direkt beim Testen sichtbar, ohne die DB inspizieren zu müssen. Aufbau (`debug_deck_line()` in `drill.php`):
    ...
    - **R (Reserve)** = wartende Karten (`reserve_known` + `reserve_new`), sinkt mit jedem Nachschub ins Deck. Steht bewusst als eigene Zahl da (nicht in T eingerechnet) — zeigt, wie viel Vorrat die Liste noch hat.
