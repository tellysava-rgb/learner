...
- **Admin-only Abschnitte** _(v3.2.16)_: "Für Admins: Einstellungen & Benutzerverwaltung" und "Für Technik-Fans: Karten per KI-Agent verwalten" sind nur für Personen mit Admin-Status sichtbar (`$_SESSION['is_admin']`) — dadurch **10 Abschnitte für Admins, 8 für alle anderen** _(Stand v3.3.2)_
...
- **Live-Ausprobieren im Aussprache-Abschnitt** _(v3.3.3)_: Button "Anhören" nutzt dieselbe `speakWord()`-Logik (Web Speech API) wie Leitner-/Drill-Karten, spricht den Beispieltext "Mich kann man hören" auf `de-CH`. Begriff und verwendete Sprache stehen direkt daneben als Text.
