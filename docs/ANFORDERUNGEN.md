...
- **Wirkung:** In `learn.php` und `drill.php` erscheint nach jeder beantworteten Karte ein `alert alert-info`-Panel (einmalig, wie eine Flash-Message) mit dem Vorher/Nachher-Status der gerade beantworteten Karte:
  ...
  - Drill, feste zweite Zeile in jeder Variante: "Anzahl in der Session: X" — aktuelle Grösse des Session-Decks (bekannte + neue + vorgemerkte Karten), erfasst **nach** einem eventuellen Nachschub aus der Reserve (siehe "Aktiver Pool an Session-Länge gekoppelt") — macht die Pool-Begrenzung/den Nachschub direkt beim Testen sichtbar, ohne die DB inspizieren zu müssen _(v3.7.2)_
