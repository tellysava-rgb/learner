...
- Öffentliche Listen anderer Personen sind über den Link **"Entdecken"** neben "Leitner"/"Drill"/"Meine Listen"/"Statistik" erreichbar, der auf die Galerie in `discover.php` führt _(bis v3.18.x war eine Vorschau von bis zu 6 öffentlichen Listen direkt auf der Startseite eingebettet — seit dem Redesign ausgelagert, siehe Abschnitt "Öffentliche Listen entdecken", um die Startseite übersichtlich zu halten)_
- **Globale Leitner-/Drill-Buttons abhängig vom Sprachlevel** _(v3.19.0)_: Die listenübergreifenden Buttons "Leitner"/"Drill" (siehe unten) werden ausgeblendet, sobald irgendeine aktive Liste der Person ein A1/A2-Sprachlevel hat (`person_has_beginner_active_list()` in `includes/levels.php`, Level pro Sprache aus `profile.php`, ohne Eintrag gilt A1). Ziel: Anfänger lernen gezielt pro Deck statt mehrere Sprachen in einer gemeinsamen Session zu mischen. Die Pro-Liste-Buttons im Footer jeder Listen-Karte bleiben davon unabhängig **immer sichtbar**
...
- `discover.php` **ohne** `list_id`: filterbare **Galerie aller öffentlichen Listen** anderer Personen (Name, Beschreibung, Besitzer, Sprachen, Anzahl Karten) _(v3.19.0 — bis dahin zeigte die Startseite eine eingebettete Vorschau von bis zu 6 Listen, ohne eigenen Überblick; `discover.php` ohne `list_id` leitete zur Startseite weiter)_
  - **Filter nach Ausgangs-/Zielsprache**: zwei `<select>`-Dropdowns mit den tatsächlich in öffentlichen Listen vorkommenden Sprachen, per GET-Parameter (`lang_a`/`lang_b`) — bookmarkbar, kein Formular-POST nötig
  - Jede Kachel hat einen Button **"Vorschau"** (bis v3.18.x: "Vorschau & Kopieren") → öffnet `discover.php?list_id=X`
- `discover.php` **mit** `list_id`: **Vorschau** aller Karten (Sprache A + Sprache B) inkl. Tags (Badges, gleiche Darstellung wie in `edit.php`, seit v3.19.0). Vorhandene Lautschrift (`phonetic_b`) wird dabei bereits in der Vorschau angezeigt (in eckigen Klammern hinter dem Begriff, z.B. `cinquante-neuf [sɛ̃kˈɑ̃tnˈœf]`) — gleiche Darstellung wie in der Kartenübersicht (`edit.php`) _(v3.3.13)_
- Button **"Kopieren & als eigene Liste übernehmen"** → Liste wird als eigene unabhängige Kopie übernommen (inkl. Tags, siehe Abschnitt "Tags pro Karte")
...
- activated_on      → Datum, an dem die Karte aus der Warteschlange aktiviert wurde (v3.17.0).
                      Einzige Grundlage für "wie viele neue Karten wurden heute schon aktiviert"
                      (Tageslimit). NULL bei Karten, die vor v3.17.0 aktiviert wurden.
...
- **Gezählt wird über das Aktivierungsdatum `activated_on`, nicht über den Lernzustand** _(Bugfix v3.17.0)_: Bis dahin wurde "heute schon aktiviert" indirekt aus `status='active' AND next_due_date = heute AND leitner_box = 1` erschlossen. Dieser Hilfsindikator verschwindet, sobald die Karte beantwortet wird (sie verlässt Fach 1 bzw. bekommt ein späteres Fälligkeitsdatum) — nach einer abgeschlossenen Session war das Tageslimit dadurch wieder "frei", und jede weitere Session desselben Tages aktivierte erneut bis zu `DAILY_CARD_LIMIT` neue Karten (gemessen: 3 Sessions an einem Tag = 30 statt 10 neue Karten). Mit `activated_on` ist die Zählung unabhängig vom weiteren Lernverlauf.
...
- **Verfügbarkeits-Hinweis auf der Leitner-Setup-Seite** _(v3.3.18, Formulierung vereinfacht v3.3.19, als Infobox v3.3.20, Tageslimit-Verbrauch ergänzt v3.3.21, nur bei tatsächlicher Drosselung sichtbar v3.3.22, unterscheidet Tageslimit vs. leere Warteschlange v3.3.24)_: Unter dem Feld "Kartenanzahl" steht als eigene Infobox (`alert alert-info`, wie andere Meldungen in der App), die den tatsächlich zutreffenden Engpass benennt — es gibt zwei unabhängige, sich gegenseitig ausschliessende Gründe, warum weniger als das Tageslimit aus der Warteschlange kommt:
  ...
  **Zu klein eingestellte Kartenanzahl** _(v3.17.0)_: Ist die Kartenanzahl KLEINER als die heute verfügbare Zahl, erscheint die Box ebenfalls — mit umgekehrter Aussage: "Heute stehen X Karten an (Y fällig + Z neu aus der Warteschlange). Mit der eingestellten Kartenanzahl bearbeitest du davon R — N Karten bleiben danach für heute fällig." Vorher blieb dieser Fall komplett unkommentiert, obwohl der Hinweistext unter dem Feld "App zeigt alle fälligen Karten" verspricht: die Session liess dann stillschweigend Karten für heute liegen, und die Startseite meldete nach dem Durcharbeiten weiterhin fällige Karten. Die Box ist damit nur noch unsichtbar, wenn Kartenanzahl und verfügbare Menge exakt übereinstimmen.
...
- **Abschluss-Abgleich** _(v3.17.0)_: Eine Session ist eine **Momentaufnahme** — die Queue wird beim Start einmalig gebaut und ist auf die gewählte Kartenanzahl begrenzt, während die Zahl "heute fällig" auf der Startseite live berechnet wird. Beides kann auseinanderlaufen (z.B. Kartenanzahl kleiner als die Menge fälliger Karten). Am Session-Ende wird deshalb gegen denselben Massstab nachgezählt (`count_due_cards()`, identische Bedingung wie beim Queue-Aufbau, im Themen-Modus zusätzlich auf die getaggten Karten eingeschränkt). Sind noch Karten für heute offen, erscheint auf der Abschluss-Seite ein Hinweis "Noch N Karten heute fällig" samt Erklärung, und der Button heisst **"Weiterlernen"** statt "Neue Session". Damit kann kein Restbestand mehr stillschweigend liegen bleiben — auch nicht aus Ursachen, die heute noch nicht bekannt sind. Grund für die Einführung: mehrfach gemeldetes "ich habe alles durchgearbeitet, trotzdem ist noch etwas fällig", das ohne diesen Abgleich wie ein Widerspruch zur Startseite wirkte
- **Datumsbasis beim Beantworten ist der reale Kalendertag** _(Bugfix v3.17.0)_: Die Fälligkeits-Berechnung (`heute + Intervall`, "falsch → morgen") verwendete den beim **Session-Start** ermittelten Tag. Bei einer Session über Mitternacht hinweg fiel "morgen" dadurch auf den bereits laufenden Tag — die Karte war unmittelbar nach der Antwort wieder "heute fällig". Es gilt jetzt durchgängig der reale Tag zum Zeitpunkt der Antwort, auch für `learning_events.learn_date` (sonst zählte die Antwort für Streak/Statistik auf den Vortag)
...
- **`install.php` stempelt die `db_version`** _(Bugfix v3.17.0)_: Nach dem Anlegen der Tabellen schreibt `install.php` die höchste bekannte Migrations-ID (`latest_migration_id()` aus `migrations.php`) in `settings.db_version`. Vorher fehlte dieser Eintrag komplett — `run_pending_migrations()` startete deshalb bei `db_version = 0` und liess anschliessend alle `ALTER TABLE ... ADD COLUMN`-Migrationen auf Spalten los, die `install.php` gerade erst angelegt hatte. Das brach mit "Duplicate column" ab und legte eine frische Installation bei **jedem** Request lahm (betraf bereits Migration 15). Die Migrations-Liste liegt dafür in einer eigenen Funktion `migration_definitions()`, damit `install.php` die höchste ID abfragen kann, ohne Migrationen auszuführen
