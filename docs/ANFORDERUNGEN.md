...
- **Lautsprecher-Fix für iOS/Android** _(v3.6.0, korrigiert v3.7.3)_: `speechSynthesis` spielt auf iOS/Android sonst nur über Kopfhörer bzw. Ohrhörer statt über den Lautsprecher, da die Audio-Session des Geräts dafür nicht in der Kategorie "playback" läuft (in der u.a. der Stumm-Schalter des iPhones ignoriert wird). Fix in `learn.php`/`drill.php` (`unlockAudioSession()`), nur auf erkanntem iOS/Android (`navigator.userAgent`; iPadOS 13+ zusätzlich über `navigator.platform === 'MacIntel' && maxTouchPoints > 1`, da es sich als Mac meldet), nicht auf Desktop: ein stummes `<audio>`-Element (0.25 s, `data:`-URI) läuft mit `loop = true` durchgehend weiter, solange die Seite offen ist, und hält damit die Audio-Session aktiv.
...
- **Wirkung:** In `learn.php` und `drill.php` erscheint nach jeder beantworteten Karte ein `alert alert-info`-Panel (einmalig, wie eine Flash-Message) mit dem Vorher/Nachher-Status der gerade beantworteten Karte:
  ...
  - Drill, feste zweite Zeile in jeder Variante _(v3.7.2, Format überarbeitet v3.7.4)_: **"Anzahl in der Session: X (aktuell Y)"** — macht die Pool-Begrenzung und den Reserve-Nachschub direkt beim Testen sichtbar, ohne die DB inspizieren zu müssen. Aufbau (`debug_deck_line()` in `drill.php`):
    - **X = Zielgrösse**, beim Sessionstart einmalig ermittelt (`max(DRILL_MIN_ACTIVE_CARDS, Minuten × DRILL_CARDS_PER_MINUTE)`) und über die ganze Session **konstant** — bewusst so, damit erkennbar bleibt, worauf die Session ursprünglich ausgelegt war.
    - **Y = aktueller Stand** der tatsächlich rotierenden Karten (`pool_known` + `pool_new`), erfasst **nach** einem eventuellen Nachschub aus der Reserve. Weicht erst von X ab, wenn die Reserve erschöpft ist und weitere Karten gemeistert bzw. als zu schwer markiert wurden.
    - **Vorgemerkte Karten stehen separat dahinter** ("… aktuell 5 + 14 vorgemerkt"), da sie **nicht** gegen die Zielgrösse begrenzt werden — ein unerwartet grosses Deck lässt sich damit sofort auf Vormerkungen zurückführen. Ohne Vormerkungen entfällt der Zusatz.
    - Sessions, die vor v3.7.1 gestartet wurden, kennen keine Zielgrösse — dort wird nur der aktuelle Stand ohne Klammer gezeigt.
