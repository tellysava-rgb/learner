...
[ ] Drill, normale Karte (nicht vorgemerkt): Panel zeigt bei einer normalen Antwort BEIDE Zähler gleichzeitig, je auf eigener Zeile und mit Bedeutungs-Suffix — "Mastery-Zähler X/3 - gewusst" und "Zu-schwer-Zähler X/5 - nicht gewusst"; eine separate "gewusst"/"musste nachdenken"-Zeile gibt es im Normalfall NICHT mehr. Wird die Karte in diesem Schritt gemeistert oder als zu schwer markiert, zeigt das Panel stattdessen die Antwort-Zeile plus Fach-Änderung bzw. den Hinweis "als zu schwer markiert". _(v3.2.42, Suffix-Format v3.7.8)_
